...
1. 选择要新增的变量，例如I/O变量，会打开如下新增弹窗。新增页面，默认只显示“**基本信息**”和“**值**”这两个配置信息，其他配置均处于未启用状态，在弹窗顶部 开启对应的配置后，才会在下图的新增界面上显示对应的配置项信息。

    ![alt text](4.png)

2. 设置各配置项，设置完成点击右下角的“确认”按钮，该变量将显示在资产窗口。

**说明**：如果配置了某个配置项（例如：历史存储）后又将其关闭并保存了变量， 则下次在该变量上再次启用该配置时，会自动显示上次已配置的内容。 
