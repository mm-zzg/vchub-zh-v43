...
在WAGO SCADA系统中，以变量作为媒介，通过各种通用的工业协议与实际的生产设备建立通讯，达到设备检测、下发指令的目的。

变量是一个用于标识实时数据或配置信息的抽象概念。它可以代表一个传感器的测量值、一个设备的状态、或其它与设备相关的数据。

资产是变量的顶部节点，WAGO SCADA系统中，可以有任意数量的资产，用户可以将变量创建到任意资产中进行维护，每个资产中的变量相互隔离，变量依托于资产存在于系统中。

可以认为，变量是资产的原子级对象，而资产是变量的容器，一个资产中，可以创建多个变量。（基于性能以及合理的分组机制，单个资产中建议变量的数量不超过 **3万** 个变量）

为了适用于复杂的实际生产环境，WAGO SCADA引入了 **模型** 与 **实例** 概念，它们与变量一样位于资产的子集中，目的是为了让用户更方便的对真实的设备进行建模，复用该模型，实例出多个设备实例。
...
这里所提到的模型、实例、变量都归属于一个资产，资产间的数据相互隔离，并不能相互引用，这里的例子，主要用于理解WAGO SCADA系统为什么要在变量之上提出资产的概念，具体使用请参照本模块相关功能具体操作手册的介绍。