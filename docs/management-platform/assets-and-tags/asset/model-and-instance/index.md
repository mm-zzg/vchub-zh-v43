...
WAGO SCADA中的模型和实例提供了面向对象的数据设计能力。通过定义模型并生成一个或多个实例，可以快速构建复杂的组态画面。所有实例都会继承模型中的数据配置更改，从而显著减少了数据模型组态的工作量。

在WAGO SCADA中，模型中的变量不会执行任何操作，可以将模型认为是设备模板。我们可以将模型实例化，实例化后的数据会进行数据推送和订阅。
...
用户可以在 [资产树](../asset-tree/index.md) 中管理模型和实例，如下图所示，分为模型和实例两个选项卡，用户可以在模型和实例各自的选项卡列表中管理它们。
