...
WAGO SCADA 提供Windows 64位操作系统安装包。
...
5. 选择安装位置，默认路径：“C:\Program Files\WAGO SCADA”

   ![alt text](8.png)

6. 选择 WAGO SCADA 应用程序数据目录
...
1. 创建专用服务账号
   ...
   在Windows本地用户和组中新建一个账号（如 WagoSCADASvc）：

   ![alt text](14.png)

2. 设置 **服务安装目录** 权限

   进入服务安装目录（例如 C:\Program Files\WAGO SCADA），点击鼠标右键，选择“属性“ → “安全“

   - 根据实际安全需求选择保留的用户或组，删除不必要的用户或组（如 Users、Everyone）。

   - 添加 WagoSCADASvc 用户，并为其赋予读取、写入、修改权限。
...
3. 修改服务运行账号
   ...
   在服务管理器（services.msc）中找到 WAGO SCADA服务 → 右键菜单中点击"属性" →"登录" → 选择“此账户”：
...
4. 设置 **应用程序数据目录** 权限
   ...
   进入安装时选择的应用程序数据目录（例如 C:\ProgramData\WAGOSCADA），右键菜单中点击"属性" →"安全"：

   - 根据实际安全需求选择保留的用户或组，删除不必要的用户或组（如 Users、Everyone）。

   - 添加 WagoSCADASvc 用户，赋予读取、写入、修改权限。
...
1. 从控制面板进入软件卸载列表，找到WAGO SCADA进行卸载。
