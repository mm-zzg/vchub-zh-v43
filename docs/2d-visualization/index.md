...
与其他图片类型相比，可缩放矢量图片 （SVG） 具有多个优点。因为它们是矢量图，所以可以在不损失清晰度或分辨率的情况下进行缩放。要在 WAGO SCADA中使用图片，只需将图片直接拖到您希望它出现的画面位置。

![alt text](5.png)

## 绑定

绑定是一种机制，它允许控件上的属性根据WAGO SCADA中其他位置的值的更改而更改。例如，Tag 的值可以绑定到仪表或数码管上。绑定的力量来自于绑定类型的多样性。
